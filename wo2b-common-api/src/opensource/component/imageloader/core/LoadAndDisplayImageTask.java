...
import opensource.component.imageloader.core.assist.FailReason.FailType;
import opensource.component.imageloader.core.decode.ImageDecoder;
import opensource.component.imageloader.core.decode.ImageDecodingInfo;
import opensource.component.imageloader.core.download.ImageDownloader;
import opensource.component.imageloader.core.download.ImageDownloader.Scheme;
import opensource.component.imageloader.core.imageaware.ImageAware;
import opensource.component.imageloader.utils.IoUtils;
 ...
 * display it in {@link opensource.component.imageloader.core.imageaware.ImageAware} using {@link DisplayBitmapTask}.
 *
...
final class LoadAndDisplayImageTask implements Runnable {
	...
	private File getImageFileInDiscCache() {
		//古天乐、刘青云一起出现时，屏幕有点暗--扫毒。
		// NOTICE: custom-made.
		String extraDir = options.getSaveImageOptions().getExtraDir();
		DiscCacheAware discCache = configuration.discCache;
		File imageFile = discCache.get(extraDir, uri);
		File cacheDir = imageFile.getParentFile();
		if (cacheDir == null || (!cacheDir.exists() && !cacheDir.mkdirs())) {
			imageFile = configuration.reserveDiscCache.get(extraDir, uri);
